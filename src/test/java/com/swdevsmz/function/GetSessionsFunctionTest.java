...
import com.swdevsmz.function.GetUser;
...
class GetSessionsFunctionTest {
        ...
        private static GetUser getSessions;
        ...
        public static void setup() {
                ...
                attributes.add(
                                AttributeDefinition.builder().attributeType(ScalarAttributeType.S)
                                                .attributeName("userId").build());
                attributes.add(
                                AttributeDefinition.builder().attributeType(ScalarAttributeType.S)
                                                .attributeName("sessionId").build());
                ...
                List<User> sessions = new ArrayList<>();
                sessions.add(new User("test-def456", Instant.now().getEpochSecond(),
                                List.of("player1", "player2", "player3"),
                                Map.of("player1", 6, "player2", 3, "player3", 7)));
                sessions.add(new User("test-abc123", Instant.now().getEpochSecond(),
                                List.of("player1", "player3", "player4"),
                                Map.of("player1", 2, "player3", 7, "player4", 9)));
                sessions.add(new User("test-ghi789", Instant.now().getEpochSecond(),
                                List.of("player3", "player5", "player12"),
                                Map.of("player3", 3, "player5", 4, "player12", 10)));

                // For each session, create a table row for each userId. This will create
                // duplicate data but allow us to
                ...
                sessions.forEach(session -> {
                        session.getParticipants().forEach(participant -> {
                                ...
                                values.put("sessionId", AttributeValue.builder().s(session.getUserId()).build());
                                // n = Number
                                values.put("timestamp", AttributeValue.builder()
                                                .n(String.valueOf(session.getTimestamp())).build());
                                // ss = String Set/List
                                values.put("participants",
                                                AttributeValue.builder().ss(session.getParticipants()).build());

                                Map<String, AttributeValue> restaurants = new HashMap<>();
                                session.getResults().forEach(
                                                (key, value) -> restaurants.put(key,
                ...
                getSessions = new GetUser(tableName, dynamoDbClient);
        }
